// $Header: /u/drspeech/repos/quicknet2/QN_HTKstream.cc,v 1.12 2011/09/06 22:12:05 davidj Exp $
...
QN_InFtrStream_HTK::QN_InFtrStream_HTK(int a_debug, 
    ...
    : log(a_debug, "QN_InFtrStream_HTK", a_dbgname), 
      ...
      compBiases(NULL)
{
    log.log(QN_LOG_PER_SENT, "Creating InFtrStream_HTK from file '%s'.",
	    QN_FILE2NAME(file));
	...
	build_index();
	if (n_segs>1) {
	  // only log if n_segs>1 otherwise probably in a list
	  // and a large number of messages get generated
...
QN_SegID QN_InFtrStream_HTK::nextseg() {
    ...
    } else if (current_seg==-1) {
      // list making can rewind segment, so skip header again
      QNInt32 hdr[3];
      int ec;

      ec = fread((char *)hdr, sizeof(QNUInt32), 3, file);
      if (ec < 3) {
	log.error("Error reading HTK header: '%s'",QN_FILE2NAME(file));
      } else {
	++current_seg;
	rc= QN_SEGID_UNKNOWN;
      }
