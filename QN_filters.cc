const char* QN_filters_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_filters.cc,v 1.29 2011/08/16 19:42:30 davidj Exp $";
...
QN_InFtrStream_Pad::QN_InFtrStream_Pad(int a_debug,
				       const char* a_dbgname,
				       QN_InFtrStream& a_str,
				       size_t a_head_pad, size_t a_tail_pad,
				       QN_PadType a_pad_type)
  : log(a_debug, "QN_InFtrStream_Pad", a_dbgname),
    str(a_str),
    head_pad(a_head_pad),
    tail_pad(a_tail_pad),
    pad_type(a_pad_type),
    n_ftrs(a_str.num_ftrs()),
    end_marker(0)
{
    my_seg = QN_SIZET_BAD;
    my_frameno = QN_SIZET_BAD;
    pad_buf = new float[n_ftrs];
    frame_buf = new float[n_ftrs];
    char* pad_type_string;
    switch (pad_type)
     {
    case QN_PAD_ZERO:
	pad_type_string = "zero";
	break;
    case QN_PAD_REPEAT:
	pad_type_string = "frame repeat";
	break;
    default:
	log.error("unknown pad type");
    }
    log.log(QN_LOG_PER_RUN, "Creating padded stream with %lu head frames and"
	    " %lu tail frames using %s padding.", (unsigned long) head_pad,
	    (unsigned long) tail_pad, pad_type_string);
}

QN_InFtrStream_Pad::~QN_InFtrStream_Pad()
{
    delete[] frame_buf;
    delete[] pad_buf;
}
...
inline size_t
QN_InFtrStream_Pad::num_ftrs()
{
    return str.num_ftrs();
}

inline size_t
QN_InFtrStream_Pad::num_segs(void)
{
    return str.num_segs();
}


QN_SegID QN_InFtrStream_Pad::nextseg(void) {
	...
	my_frameno = 0;
	end_marker = 0;	// Do not know how many frames this seg yet.

	// Read the first frame so we can use it for padding if necessary.
	size_t count;
	count = str.read_ftrs(1, frame_buf);
	if (count!=1)
	    log.error("<1 frame in a segment.");
	// Work out the padding.
	switch(pad_type)
	{
	case QN_PAD_ZERO:
	    qn_copy_f_vf(n_ftrs, 0.0f, pad_buf);
	    break;
	case QN_PAD_REPEAT:
	    qn_copy_vf_vf(n_ftrs, frame_buf, pad_buf);
	    break;
	default:
	    log.error("Invalid pad type in nextseg().");
	}
    }
...
size_t QN_InFtrStream_Pad::read_ftrs(size_t a_frames, float* a_ftrs)
{
    size_t frames_left = a_frames;
    size_t frames_done = 0;
    size_t cnt;

    // First do the padding
    if (my_frameno<head_pad)
    {
	while (frames_left>0 && (my_frameno < head_pad))
	{
	    qn_copy_vf_vf(n_ftrs, pad_buf, a_ftrs);
	    my_frameno++;
	    frames_left--;
	    frames_done++;
	    a_ftrs += n_ftrs;
    ...
    // Then special-case the first frame in the segment
    if ((frames_left>0) && (my_frameno==head_pad))
    {
	qn_copy_vf_vf(n_ftrs, frame_buf, a_ftrs);
	my_frameno++;
	frames_left--;
	frames_done++;
	a_ftrs += n_ftrs;
    }
    // Then do the body of the segment.
    if (end_marker==0)	// end_marker is only set at end
    {
	cnt = str.read_ftrs(frames_left, a_ftrs);
	my_frameno += cnt;
	frames_left -= cnt;
	frames_done += cnt;
	a_ftrs += n_ftrs * cnt;
	...
	if (frames_left>0)
	{
	    // We are here if we have run out of frames in this seg
	    end_marker = my_frameno;
	    switch(pad_type)
	    {
	    case QN_PAD_ZERO:
		qn_copy_f_vf(n_ftrs, 0.0f, pad_buf);
		break;
	    case QN_PAD_REPEAT:
		qn_copy_vf_vf(n_ftrs, a_ftrs-n_ftrs, pad_buf);
		break;
	    default:
		log.error("Invalid pad type in read_ftrs().");
	    }
	}
    }
    ...
    if (end_marker>0)
    {
	while (frames_left>0 && my_frameno<(end_marker+tail_pad) )
	{
	    qn_copy_vf_vf(n_ftrs, pad_buf, a_ftrs);
	    my_frameno++;
	    frames_left--;
    ...
    // Then do any tail padding.
    return frames_done;
}

int QN_InFtrStream_Pad::rewind(void)
{
    my_seg = QN_SIZET_BAD;
    my_frameno = 0;
    return str.rewind();
...
QN_SegID QN_InFtrStream_Pad::set_pos(size_t a_segno, size_t a_frameno)
{
    size_t str_frameno;

    if (a_frameno != 0)
	log.error("cannot seek with segment in set_pos()");
    my_seg = a_segno;
    my_frameno = 0;
    end_marker = 0;
     QN_SegID segid = str.set_pos(my_seg,0);
     if (segid == QN_SEGID_BAD) {
	 log.error("unable to seek utt %lu in set_pos()",
		   (unsigned long) a_segno);
     }
     return segid;
}

int QN_InFtrStream_Pad::get_pos(size_t* a_segnop, size_t* a_framenop)
{
    if (a_segnop!=NULL)
	*a_segnop = my_seg;
    if (a_framenop!=NULL)
	*a_framenop = my_frameno;
    return QN_OK;
}

size_t QN_InFtrStream_Pad::num_frames(size_t a_segno){
    size_t unpadded = str.num_frames(a_segno);
    if (unpadded==QN_SIZET_BAD)
	return QN_SIZET_BAD;
    if (a_segno==QN_SIZET_BAD) {
	return unpadded + str.num_segs()*(head_pad+tail_pad);
    } else {
	return unpadded + head_pad + tail_pad;
    }
}

////////////////////////////////////////////////////////////////
// Below is the buffered pad stream by Arlo Faria that was present in
// some versions of quicknet.
// QN_InLabStream_Pad::QN_InLabStream_Pad(int a_debug,
// 				       const char* a_dbgname,
// 				       QN_InLabStream& a_str,
// 				       int a_pad)
//   :
//   log(a_debug, "QN_InLabStream_Pad", a_dbgname),
//   str(a_str),
//   pad(a_pad),
//   num_labels(a_str.num_labs())
// {
//   my_seg = QN_SIZET_BAD;
//   my_frameno = QN_SIZET_BAD;
//   num_frames_unpadded = QN_SIZET_BAD;
//   num_frames_padded = QN_SIZET_BAD;

//   buffer_size = 1;  
//   buffer_size += (pad>0) ? 2*pad : -pad;  // extra space for padding
//   utt_buffer = new QNUInt32 [buffer_size * num_labels];
// }

// QN_InLabStream_Pad::~QN_InLabStream_Pad()
// {
//   delete [] utt_buffer;
// }

// size_t QN_InLabStream_Pad::num_labs()
// {
//   return num_labels;
// }

// int QN_InLabStream_Pad::rewind()
// {
//   my_seg = QN_SIZET_BAD;
//   my_frameno = 0;
//   return str.rewind();
...
// QN_SegID QN_InLabStream_Pad::nextseg()
// {
//   QN_SegID segid = str.nextseg();
//   if (segid == QN_SEGID_BAD) {
//     my_seg = QN_SIZET_BAD;
...
// void QN_InLabStream_Pad::fill_utt_buffer(void) {
//   // If all sub-streams supported num_frames(), it would be
//   // much easier to allocate enough memory for utt_buffer.
//   // Instead, we grow it as necessary.

//   num_frames_unpadded = 0;
//   if (pad>0) {
//     // Read data into utt_buffer
//     size_t read_size = buffer_size - 2*pad;
//     QNUInt32* read_loc = utt_buffer + pad*num_labels;
//     size_t frames_read = str.read_labs(read_size, read_loc);
//     while (frames_read == read_size) {
//       // Increase buffer size
//       size_t new_buffer_size = buffer_size + read_size;
//       QNUInt32* new_utt_buffer = new QNUInt32 [new_buffer_size*num_labels];
//       // Move data to new buffer
//       qn_copy_vi32_vi32(buffer_size*num_labels, (QNInt32*) utt_buffer, (QNInt32*) new_utt_buffer);
//       delete utt_buffer;
//       // Updates
//       read_loc = new_utt_buffer + (read_loc - utt_buffer) + read_size*num_labels;
//       buffer_size = new_buffer_size;   
//       utt_buffer = new_utt_buffer;
//       num_frames_unpadded += read_size;
//       // Read more data
//       frames_read = str.read_labs(read_size, read_loc);
//     };
//     num_frames_unpadded += frames_read;
//     num_frames_padded = num_frames_unpadded + 2*pad;    

//     // Replicate end frames
//     for (int i=0; i<pad; i++) {
//       qn_copy_vi32_vi32(num_labels, 
// 			(QNInt32*) utt_buffer + pad*num_labels, 
// 			(QNInt32*) utt_buffer + i*num_labels);
//       qn_copy_vi32_vi32(num_labels, 
// 			(QNInt32*) utt_buffer + (pad+num_frames_unpadded-1)*num_labels, 
// 			(QNInt32*) utt_buffer + (pad+num_frames_unpadded+i)*num_labels);
//     }
//   } else {  // (pad<=0)
//     // Skip first frames
//     if (str.read_labs(-pad,utt_buffer)!=-pad) {
// 	log.error("Segment %lu is too short to chop left frames.", my_seg);
//     }
//     num_frames_unpadded += -pad;
//     // Read data into utt_buffer
//     size_t read_size = buffer_size;
//     QNUInt32* read_loc = utt_buffer;
//     size_t frames_read = str.read_labs(read_size, read_loc);
//     while (frames_read == read_size) {
//       // Increase buffer size
//       size_t new_buffer_size = buffer_size + read_size;
...
//     };
//     num_frames_unpadded += frames_read;
//     if (num_frames_unpadded < 1 -2*pad) {
//       log.error("Segment %lu is too short to chop right frames.", my_seg);
//     }
//     num_frames_padded = num_frames_unpadded + 2*pad;    
//   }
// }

// size_t QN_InLabStream_Pad::read_labs(size_t a_frames, QNUInt32* a_labs)
// {
//   // Determine how much data to read
//   size_t count;
//   size_t frames_left = num_frames_padded - my_frameno;
//   if (a_frames <= frames_left) {
//     count = a_frames;
//   } else {
//     count = frames_left;
//   }
  
//   // Read data from the padded utt_buffer
//   qn_copy_vi32_vi32(num_labels*count, (QNInt32*) utt_buffer+my_frameno*num_labels, (QNInt32*) a_labs);	
//   my_frameno += count;

//   return count;
// }

// size_t QN_InLabStream_Pad::num_segs()
// {
//     return str.num_segs();
// }

// size_t QN_InLabStream_Pad::num_frames(size_t a_segno)
// {
//   size_t unpadded = str.num_frames(a_segno);
//   if (unpadded==QN_SIZET_BAD)
//     return QN_SIZET_BAD;
//   if (a_segno==QN_SIZET_BAD) {
//     return str.num_frames(a_segno) + str.num_segs()*2*pad;
//   } else {
//     return str.num_frames(a_segno) + 2*pad;
//   }

// }

// int QN_InLabStream_Pad::get_pos(size_t* a_segnop, size_t* a_framenop)
// {
//   if (a_segnop!=NULL)
//     *a_segnop = my_seg;
//   if (a_framenop!=NULL)
...
// QN_SegID QN_InLabStream_Pad::set_pos(size_t a_segno, size_t a_frameno)
// {
//   my_seg = a_segno;
//   my_frameno = a_frameno;

//   // Fill the current utterance buffer
//   QN_SegID segid = str.set_pos(my_seg,0);
//   if (segid == QN_SEGID_BAD) {
...
// ////////////////////////////////////////////////////////////////
// QN_InFtrStream_NormSpkr::QN_InFtrStream_NormSpkr(int a_debug, 
// 						 const char* a_dbgname,
// 						 QN_InFtrStream& a_str,
...
//   : QN_InFtrStream_Norm(a_debug, a_dbgname, a_str, 0, 0) {
//   my_seg = QN_SIZET_BAD;
//   num_ftrs = str.num_ftrs();
  
//   // Read normalization statistics -- could be huge!
//   QN_InFtrStream* norm_str = &a_norm_str;
//   norm_str->rewind();
//   if (norm_str->num_ftrs()!=num_ftrs)
//     log.error("Dimension in norm stream does not match feature stream!");
//   size_t num_spkrs = norm_str->num_segs();
//   spkr_means = new float [num_spkrs * num_ftrs];
//   spkr_sdevs = new float [num_spkrs * num_ftrs];
//   for (size_t spkrno=0; spkrno<num_spkrs; spkrno++){
//     if (norm_str->nextseg()==QN_SEGID_BAD)
//       log.error("Norm stream should not have ended so soon!");
//     if (norm_str->read_ftrs(1,spkr_means+spkrno*num_ftrs)!=1)
//       log.error("Norm stream did not read mean vector for spkr %lu.", spkrno);
//     if (norm_str->read_ftrs(2,spkr_sdevs+spkrno*num_ftrs)!=1)
//       log.error("Norm stream did not read sdev vector for spkr %lu.", spkrno);
//   }

//   // Read speaker labels
//   QN_InLabStream* lab_str = &a_lab_str;
//   lab_str->rewind();
//   size_t num_segs = str.num_segs();  // get from feature stream, not label stream!
//   spkr_labs = new size_t [num_segs];
//   for (size_t segno=0; segno<num_segs; segno++) {
//     if (lab_str->nextseg()==QN_SEGID_BAD)
//       log.error("Speaker label stream should not have ended so soon!");
//     if (lab_str->read_labs(2,spkr_labs+segno)!=1)
//       log.error("Speaker label stream did not read label for segno %lu.", segno);
//   }
// }
// QN_InFtrStream_NormSpkr::~QN_InFtrStream_NormSpkr(void) {
//   delete [] spkr_means;
//   delete [] spkr_sdevs;
//   delete [] spkr_labs;
// }
...
// // We only have to specialize the fns that change the seg
// QN_SegID QN_InFtrStream_NormSpkr::nextseg(void) {
//   QN_SegID segid = QN_InFtrStream_Norm::nextseg();
//   if (segid == QN_SEGID_BAD) {
//     my_seg = QN_SIZET_BAD;
//   } else {
...
//       ++my_seg;
//     }
//     size_t spkrno = spkr_labs[my_seg];
//     bias_vec = spkr_means + spkrno*num_ftrs;
//     scale_vec = spkr_sdevs + spkrno*num_ftrs;
//   }
//   return segid;
// }

// int QN_InFtrStream_NormSpkr::rewind(void) {
//   my_seg = QN_SIZET_BAD;
//   return QN_InFtrStream_Norm::rewind();
// }

// QN_SegID QN_InFtrStream_NormSpkr::set_pos(size_t segno, size_t frameno) {
//   if (segno != my_seg) {
//     my_seg = segno;
//     if (QN_InFtrStream_Norm::set_pos(my_seg, 0) == QN_SEGID_BAD) {
//       log.error("unable to seek utt %d in set_pos", my_seg);
