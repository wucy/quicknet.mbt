...
const char* qnmultifwd_rcsid = 
    "$Header: /u/drspeech/repos/quicknet2/qnmultifwd.cc,v 1.13 2011/09/06 22:12:05 davidj Exp $";
#endif
...
static struct {
    ...
    QN_Arg_ListStr mlp_hidden_type;
    //cz277 - device select
    int device_no;
    //cz277 - outmap
...
{ "ftr1_format", "Main feature file format [pfile,pre,onlftr,lna,srifile,srilist,htk]", QN_ARG_STR,
  &(config.ftr1_format) },
{ "ftr1_width", "Main feature file feature columns", QN_ARG_INT,
  &(config.ftr1_width) },
{ "ftr2_file", "Second input feature file", QN_ARG_STR,
  &(config.ftr2_file) },
{ "ftr2_format","Secondary feature file format [pfile,pre,onlftr,lna,srifile,srilist,htk]", QN_ARG_STR,
  &(config.ftr2_format) },
...
//cz277 - nonlinearity
{ "mlp_hidden_type", "The nonlinear type for the hidden units (per network or per hidden layer) [sigmoid,softmax,tanh,softsign,linear]", QN_ARG_LIST_STR, &(config.mlp_hidden_type) },
//cz277 - device select
...
create_fwdmlp(int debug, const char*,
	      ...
	      const char* mlp_output_type, QN_Arg_ListStr mlp_hidden_type,	//cz277 - nonlinearity, mul actv 
	      int mlp_bunch_size, int threads, int cuda, int fe, QN_MLP** mlp_ptr)
{
    ...
    if (cuda)
    {
	    ...
	    QN_cuda_init(config.device_no, config.env_var4dev_id);     //cz277 - device select
	    char devstr[128];
	    sprintf(devstr, "CUDA using device: %i.", QN_cuda_current_device());
	    QN_OUTPUT(devstr);

	    mlp3 = new QN_MLP_BunchCudaVar(-1, debug, "fwdmlp",
					   n_layers, layer_size,
					   QN_CRITERION_XENTROPY, hiddenlayer_types,	//cz277 - criteria	//cz277 - nonlinearity, mul actv
	...
	if (threads==1)
	{
	    mlp3 = new QN_MLP_BunchFlVar(-1, debug, "fwdmlp",
					 n_layers, layer_size,
					 QN_CRITERION_XENTROPY, hiddenlayer_types,	//cz277 - criteria	//cz277 - nonlinearity, mul actv
...
//				   outlayer_type, mlp_bunch_size);
	...
	else
	{
	    ...
	    {
		mlp3 = new QN_MLP_ThreadFlVar(-1, debug, "fwdmlp",
					      n_layers, layer_size,
					      QN_CRITERION_XENTROPY, hiddenlayer_types,       //cz277 - criteria      //cz277 - nonlinearity, mul actv
					      outlayer_type, mlp_bunch_size,
					      threads);
...
// Open files and provisionally check arguments.
    ...
    create_fwdmlp(debug, "mlp",
		  ...
		  config.mlp_output_type, config.mlp_hidden_type,	//cz277 - nonlinearity, mul actv
		  config.mlp_bunch_size, config.mlp_threads, 
		  config.use_cuda, config.use_fe, &mlp);
    ...
    QN_hardForward(&mapclass,    //cz277 - outmap
