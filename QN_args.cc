  ...
  while ((*argv)++,--*argc)
  {
	  ...
	  *((int *) ael->varptr) = get_bool_val(value);
      ...
      case QN_ARG_LIST_STR:
      {
          ...
          // Update the users value.
          QN_Arg_ListStr* list =
              (QN_Arg_ListStr*) (ael->varptr);
          char** list_vals;
          list_vals = new char *[count];
          list->count = count;
          list->vals = list_vals;
          // Finally read in the values from the list.
          char* tok = strtok(value, ",");
          for (i=0; i<count; i++)
          {
	      list_vals[i] = new char[64];
              if ((tok==NULL) || strcpy(list_vals[i], tok) == NULL)
              {
                  fprintf(stderr,
                          "Warning: arg %s should be a comma separated "
                          "list of integers.\n",
		...
		case QN_ARG_LIST_STR:
                {
                    int i;
                    QN_Arg_ListStr* list =
                        (QN_Arg_ListStr*) (argtab->varptr);
                    fprintf(stderr, " (");
                    for (i=0; i<list->count; i++)
                    {
                        if (i!=0)
                            fputc(',', stderr);
                        fprintf(stderr, "%s", (list->vals)[i]);
                    }
                    fprintf(stderr, ")");
                    break;
                }
