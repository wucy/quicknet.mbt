...
//cz277 - momentum
void qn_nv_multnacc2_fmfmf_mf(size_t Sk,size_t Sm,size_t Sn, float scale1,
			float scale2, const float *A,const float *B,float *C);
	
//cz277 - momentum
void qn_add_fmfmf_mf(size_t Sm,size_t Sn,const float *C1,float *C2);
...
#ifdef QN_HAVE_LIBBLAS
inline void
qn_bl_multnacc2_fmfmf_mf(const int k, const int m, const int n,
			float alpha, float beta, const float* A,
...
#else
    sgemm(QN_BLAS_NOTRANSPOSE, QN_BLAS_TRANSPOSE,
	 n, m, k, alpha, (float*) B, n, (float*)A, m, beta, C, n);
#endif
}
#endif
...
//cz277 - momentum
inline void
qn_pp_multnacc2_fmfmf_mf(size_t a_rows, size_t a_cols, size_t b_cols,
			float scale1, float scale2, const float* a, 
			const float* b, float* res)
{
    int i, j;
    for (i = 0; i < a_cols; ++i)
	for (j = 0; j < b_cols; ++j)
	    res[i * b_cols + j] *= scale2;
    qn_pp_multnacc_fmfmf_mf_sss(a_cols, a_rows, b_cols,
	a, b, res, a_cols, b_cols, b_cols, scale1);
}

//cz277 - momentum
//res = res * scale1 + a * b * scale2
inline void
qn_multnacc2_fmfmf_mf(size_t a_rows, size_t a_cols, size_t b_cols,
                     float scale1, float scale2, const float* a,
		     const float* b, float *res)
{
#ifdef QN_HAVE_LIBBLAS
    if (qn_math & QN_MATH_BL) {
        qn_bl_multnacc2_fmfmf_mf(a_rows, a_cols, b_cols, scale1, scale2, a, b, res);
    }
    else
#endif
    if (qn_math & QN_MATH_PP) {
        qn_pp_multnacc2_fmfmf_mf(a_rows, a_cols, b_cols, scale1, scale2, a, b, res);
    }
    else {
        qn_nv_multnacc2_fmfmf_mf(a_rows, a_cols, b_cols, scale1, scale2, a, b, res);
    }
}
...
//cz277 - nonlinearity
inline void
qn_softsign_vf_vf(size_t n, const float* in_vec, float* out_vec)
{
    size_t i;
    for (i = n; i != 0; --i)
    {
...
//cz277 - nonlinearity, in_vec1 = y_i, in_vec2 = t_i
inline void
qn_dxentropy_vf_vf(size_t n, const float* in_vec1, const float* in_vec2, float* out_vec)
{
    size_t i;
    for (i = n; i != 0; --i)
    {
	float yi = *in_vec1++;
	if (yi == 0)
	    yi = QN_MinProb;	//an approximate to den = (1 - QN_MinProb) * QN_MinProb
	*out_vec++ = - (*in_vec2++) / yi;
	//*out_vec++ = *in_vec2++;
    }
}
...
//cz277 - nonlinearity
inline void
qn_dsoftmax_vf_vf(size_t n, const float* in_vec, float* out_vec)
{
    size_t i;
    float yi;
    for (i = n; i != 0; --i, ++in_vec, ++out_vec)
    {
	yi = *in_vec;
	*out_vec = (1 - yi) * yi;
    }
...
inline void
qn_dsoftsign_vf_vf(size_t n, const float* in_vec, float* out_vec)
{
    /*size_t i;
...
inline void
qn_scale_fvf_vf(size_t n, const float scale, float *inout_vec)
{
	if (n > 0)
	{
		for (int i = 0; i < n; ++i)
			*inout_vec++ = scale * (*inout_vec);
	}
}
