...
void
QN_hardForward(struct MapStruct *mapptr,        //cz277 - outmap
	       int debug, const char* dbgname, int verbose, QN_MLP* mlp,
	       ...
	       size_t bunch_size, int lastlab_reject)
{
    ...
    //cz277 - outmap
    unsigned long map_right_pres = 0;
    QNUInt32 *map_inlab_buf = NULL;
	...
	do			// Iterate over all bunches in segment.
	{
		...
		if (mapptr->opt > 0)
		{
		    int reflab = -1, hyplab = -1;
		    map_inlab_buf = inlab_buf;
		    float *out_ptr = &out_buf[0];

		    for (i=0; i<inp_count; i++)
		    {
			if (mapptr->opt == 1) { //max
			    hyplab = mapptr->srclist[qn_imax_vf_u(n_outs, out_ptr)];
			} else {        //sum
			    memset(mapptr->tgtlist, 0.0, mapptr->tgtdim * sizeof(int));
			    for (int j = 0; j != mapptr->srcdim; ++j) {
				mapptr->tgtlist[mapptr->srclist[j]] += out_ptr[j];
			    }			
			    if (mapptr->opt == 3) {     //avg
			    ...
			    hyplab = qn_imax_vf_u(mapptr->tgtdim, mapptr->tgtlist);
			}
			reflab = mapptr->srclist[*map_inlab_buf];
			if (reflab == hyplab) {
			    ++map_right_pres;
			}
			++map_inlab_buf;
	...
	//cz277 - outmap
	if (mapptr->opt > 0) {
	    QN_OUTPUT("Mapped recognition accuracy: %lu right out of %lu, "
                  "%.2f%% correct.",
                  (unsigned long) map_right_pres,
                  (unsigned long) unreject_pres,
                  100.0 * (double)map_right_pres / (double)unreject_pres);
	}
