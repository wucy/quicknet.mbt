...
: ${testdata_dir:=/home/mifs/cz277/Software/installed/QuickNet/testdata}
...
${qnmultitrn:=./qnmultitrn} \
	...
	init_weight_file=${init_weight_file:="czqnmultiout.mat"} \
	...
	mlp_bunch_size=${mlp_bunch_size:=16} \
        mlp_threads=${mlp_threads:=2} \
	a_alpha_momentum=${a_alpha_momentum:=0} \
	a_min_derror_stop=${a_min_derror_stop:=0.01} \
	a_min_derror_ramp_start=${a_min_derror_ramp_start:=0.01} \
	a_min_iter_num=${a_min_iter_num:=8} \
  	mlp_hidden_out_type=${mlp_hidden_out_type:=linear} \
  	backprop_criterion=${backprop_criterion:=xentropy} \
	log_file=${log_file:=-} \
