const char* QN_utils_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_utils.cc,v 1.108 2011/09/06 22:12:05 davidj Exp $";
		     ...
		     float bias_min, float bias_max)
{
	...
	//cz277 - dnn
	assert(mlp.num_layers()<=QN_MLP_MAX_LAYERS);
	if (section==QN_LAYER2_BIAS
	    || section==QN_LAYER3_BIAS
	    || section==QN_LAYER4_BIAS
	    || section==QN_LAYER5_BIAS
	    || section==QN_LAYER6_BIAS
	    || section==QN_LAYER7_BIAS
	    || section==QN_LAYER8_BIAS
	    || section==QN_LAYER9_BIAS
	    || section==QN_LAYER10_BIAS
	    || section==QN_LAYER11_BIAS
	    || section==QN_LAYER12_BIAS
	    || section==QN_LAYER13_BIAS
	    || section==QN_LAYER14_BIAS
	    || section==QN_LAYER15_BIAS
	    || section==QN_LAYER16_BIAS
	    || section==QN_LAYER17_BIAS
	    || section==QN_LAYER18_BIAS
	    || section==QN_LAYER19_BIAS
	    || section==QN_LAYER20_BIAS )
	{
	    min = bias_min;
	    max = bias_max;
	}
...
QN_InFtrStream* QN_open_ftrstream(int debug, const char *dbgname, 
    ...
    } else if (strcmp(format, "htk")==0) {
	QN_InFtrStream_ListHTK* htk_list_str =
	    new QN_InFtrStream_ListHTK(debug, // Select debugging.
				   dbgname, // Debugging tag.
				   ftrfile,  // Input file.
				   indexed // whether indexed
		);

	size_t htk_num_ftrs = htk_list_str->num_ftrs();
	if (width!=0 && (htk_num_ftrs!=width) ) {
	    QN_ERROR(dbgname, "HTK List file '%s' has %lu features, "
		     "command line specifies %lu.", QN_FILE2NAME(ftrfile),
		     (unsigned long) htk_num_ftrs,
		     (unsigned long) width);
	}
	if (htk_num_ftrs < min_width)	{
	    QN_ERROR(dbgname, "Minimum file width of %lu features required, "
		     "HTK List file '%s' only has %lu.",
		     (unsigned long) min_width, QN_FILE2NAME(ftrfile),
		     (unsigned long)htk_num_ftrs);
	}
	ftr_str = htk_list_str;
...
//cz277 - outmap
int
QN_loadmap(const char *mappath, const char *mapopt, struct MapStruct *mapptr)
{
    int opt;
    if (strcmp(mapopt, "") == 0) {
        mapptr->opt = 0;
        return 0;
    } else if (strcmp(mapopt, "max") == 0) {
        mapptr->opt = 1;
    } else if (strcmp(mapopt, "sum") == 0) { 
        mapptr->opt = 2;
    } else if (strcmp(mapopt, "avg") == 0) {
	mapptr->opt = 3;
    } else {
	return -1;
    } 

    FILE *mapfd = fopen(mappath, "r");
    if (mapfd == NULL) {
        return -1;
    }
    fscanf(mapfd, "%d", &(mapptr->srcdim));
    fscanf(mapfd, "%d", &(mapptr->tgtdim));
    mapptr->srclist = new int[mapptr->srcdim];
    mapptr->tgtlist = new float[mapptr->tgtdim];
    mapptr->tgtcnts = new int[mapptr->tgtdim];
    memset(mapptr->tgtcnts, 0, mapptr->tgtdim * sizeof(int));
    //printf("A %d\tB %d\n", mapptr->srcdim,  mapptr->tgtdim);
    for (int i = 0; i < mapptr->srcdim; ++i) {
        int srcidx;
        int tgtidx;
        fscanf(mapfd, "%d", &srcidx);
        fscanf(mapfd, "%d", &tgtidx);
        mapptr->srclist[srcidx] = tgtidx;
	++mapptr->tgtcnts[tgtidx];
	//printf("srcidx = %d\ttgtidx = %d\n", srcidx, tgtidx);
    }
    fclose(mapfd);
    return 0;
}
