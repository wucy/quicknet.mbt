...
//const int QN_CUDA_NTHREADS = 960;
//const int QN_CUDA_NTHREADS = 256;
//const int QN_CUDA_MAXBLOCKS = 65535;
const int QN_CUDA_MAXBLOCKS = 2147483647;

const static int NTHREADS = QN_CUDA_NTHREADS;
const static int MAXBLOCKS = QN_CUDA_MAXBLOCKS;
...
__global__ void qn_kern_tempsoftmax_mf_mf(int rows, int cols, float* in_vec, float* out_vec);
__global__ void qn_kern_maxrow_vf_f(int rows, int cols, const float* in, float* res);
__global__ void qn_kern_dualmax_ff_f(int cols, int size, int incr, const float *in_vec, float *out_vec);
__global__ void qn_kern_dualsum_ff_f(int cols, int size, int incr, const float *in_vec, float *out_vec);
__global__ void qn_kern_fastcopy_vf_mf(int mat_height, int vec_len, const float*vec, float* mat, int matsize);
...
inline void qn_dev_fastsoftmax_mf_mf(int rows, int cols, const float* in_vecs, float *comp_cache, float* out_vecs)
{
    ...
    int incr = 1;
    for (int i = 0; i <= log2n; ++i, incr *= 2)
    {
       if (i == 0) {
           qn_kern_dualmax_ff_f<<<fblocks, NTHREADS>>>(cols, size, incr, in_vecs, comp_cache); 
       } else {
           qn_kern_dualmax_ff_f<<<fblocks, NTHREADS>>>(cols, size, incr, comp_cache, comp_cache);
       }
    }
    //qn_kern_maxrow_vf_f<<<pblocks, NTHREADS>>>(rows, cols, in_vecs, comp_cache);
    qn_kern_exp_f_f<<<fblocks, NTHREADS>>>(rows, cols, size, in_vecs, comp_cache, out_vecs);
    //qn_kern_tempsoftmax_mf_mf<<<pblocks, NTHREADS>>>(rows, cols, out_vecs, comp_cache);
    incr = 1;
    for (int i = 0; i <= log2n; ++i, incr *= 2)
    {
       if (i == 0) {
           qn_kern_dualsum_ff_f<<<fblocks, NTHREADS>>>(cols, size, incr, out_vecs, comp_cache);
       } else {
           qn_kern_dualsum_ff_f<<<fblocks, NTHREADS>>>(cols, size, incr, comp_cache, comp_cache);
       }
    }

    qn_kern_div_f_f<<<fblocks, NTHREADS>>>(rows, cols, size, out_vecs, comp_cache, out_vecs);
...
//cz277 - fast softmax
inline void
qn_dev_fastcopy_vf_mf(size_t mat_height, size_t vec_len,
                  const float* vec, float* mat)
{
    int matsize = mat_height * vec_len;
    int nblocks = (matsize + NTHREADS-1)/NTHREADS;

    if (nblocks>QN_CUDA_MAXBLOCKS)
        QN_ERROR("qn_fastdev_copy_vf_mf", "nblocks too large");
    qn_kern_fastcopy_vf_mf<<<nblocks, NTHREADS>>>(mat_height, vec_len, vec, mat, matsize);
}
...
inline void
qn_dev_scale_fvf_vf(size_t n, float scale, float *inout_vec)
{
	cublasSscal(n, scale, inout_vec, 1);
}
