...
//cz277 - device select, env var
void
QN_cuda_init(int devno, const char *env_var4dev_id)
{
	...
	//cz277 - env var
	const char *env_var4dev_id_value = getenv(env_var4dev_id);
	if (env_var4dev_id_value != NULL) {	//env var for device selection exists, give it the highest priority
	    devno = atoi(env_var4dev_id_value);
	}

	//cz277 - device select, env var
        if (devno >= 0) {
	    e = cudaSetDevice(devno);
	}
	int dev;
        if (devno < 0 || e!=cudaSuccess) {
	    e = cudaGetDevice(&dev);
            QN_OUTPUT("Fail to select specified device no. %d, use device no. %d instead\n", devno, dev);
        } else {
	    dev = devno;
	}
	...
	//cz277 - new gpu
	QN_OUTPUT("Device %i has %i blocks and %i threads/block, %i blocks and %i threads/block would be involved.", devno, props.maxGridSize[0], props.maxThreadsDim[0], QN_CUDA_MAXBLOCKS, QN_CUDA_NTHREADS);
