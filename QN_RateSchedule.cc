...
QN_RateSchedule_NewBoB::QN_RateSchedule_NewBoB(float start_rate,
					       ...
					       float initial_error,
					       size_t a_max_epochs,
					       int a_min_iter_num)	//cz277 - newbob
  : scale_by(a_scale_by),
    ...
    epoch(1),
    min_iter_num(a_min_iter_num)	//cz277 - newbob
{
    //cz277 - list restore
    sprintf(this->schedname, "newbob");
...
QN_RateSchedule_NewBoB::next_rate(float current_error)
{
	...
	if (ramping)
	{
            ...
            {
                if (epoch >= this->min_iter_num)    //cz277 - newbob
                    rate = 0.0f;
                else
                    rate *= scale_by;
            }
